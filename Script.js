class Contact {
  ...
  toString() {
    return `Name: ${this.getFullName()}, Address: ${this.address}, City: ${
      this.city
    }, State: ${this.state}, ZIP: ${this.zip}, Phone: ${this.phone}, Email: ${
      this.email
    }`;
  }
  ...
  validateName(name, fieldName) {
    ...
    if (!nameRegex.test(name)) {
      throw new Error(
        `${fieldName} should start with a capital letter and have at least 3 characters.`
      );
    }
  }
  ...
  validateZip(zip) {
    ...
    if (!zipRegex.test(zip)) {
      throw new Error(
        "Invalid ZIP code. It should be 5 digits or in the format 12345-6789."
      );
    }
  }
...
class AddressBook {
  ...
  addContact(contact) {
    ...
    const isDuplicate =
      this.contacts.filter(
        (existingContact) =>
          existingContact.getFullName() === contact.getFullName()
      ).length > 0;

    if (isDuplicate) {
      throw new Error(
        `Contact with name ${contact.getFullName()} already exists.`
      );
    }
  ...
  //Generic sorting function based on key (city, state, or zip).
  sortContactsBy(key) {
    if (!["city", "state", "zip"].includes(key)) {
      throw new Error("Invalid sorting key. Use 'city', 'state', or 'zip'.");
    }
    return this.contacts.sort((a, b) => a[key].localeCompare(b[key]));
  }

  // Print sorted contacts based on city, state, or zip.
  printSortedContactsBy(key) {
    console.log(`Contacts sorted by ${key}:`);
    this.sortContactsBy(key).map((contact) => console.log(contact.toString()));
  }
}
...
console.log(addressBook.findContactByName("Priyanka", "Singh"));
